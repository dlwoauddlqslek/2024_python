# 연습문제 : 2_naverApi.py 참고하여 네이버 api로 블로그에서 월드컵에 대한 검색 결과를 크롤링하여 JSON 파일로 저장
...
def getPostData( post , jsonResult , cnt ) :
    ...
    org_link = post['bloggerlink'] # 뉴스 기사 원문의 URL
...
# [code 1]
def main() :
    node = 'blog' # 1. 크롤링할 대상 [ 네이버 제공하는 검색대상 : 1.news 2.blog 3.shop 등등 ] - 공문 참고 #https://developers.naver.com/docs/serviceapi/search/search.md
    ...
    # PY 객체를 JSON형식(문자열)으로 변환 후 파일 처리, # with open() as 파일변수명: with 종료시 자동으로 파일닫기
        # 파일 쓰기 모드 객체 생성
    with open( f'{srcText}-naver-{node}.json' , 'w' , encoding='utf-8' ) as file: # 월드컵-naver-news.json
        jsonFile = json.dumps( jsonResult , indent=4 , sort_keys=True , ensure_ascii=False )
        file.write(jsonFile)
        # json.dumps() : py객체를 JSON 문자열로 반환 함수
        # json.dumps( 변환할 py객체, indent=들여쓰기수준, sort_keys=알파벳정렬, ensure_ascii=아스키문자)
    '''
    변환할 py객체: 딕셔너리 또는 리스트
    jsonResult: 이 매개변수는 JSON으로 변환하려는 Python 객체입니다. 예를 들어, Python의 딕셔너리나 리스트가 될 수 있습니다.
    indent=4: 이 옵션은 JSON 문자열의 들여쓰기 수준을 설정합니다. indent=4는 각 레벨의 들여쓰기를 4개의 공백으로 설정하여
              출력된 JSON이 더 읽기 쉬운 형태가 되도록 합니다. 생략시 들여쓰기 없음.
    sort_keys=True: 이 옵션은 JSON 객체의 키를 알파벳 순서로 정렬합니다. True로 설정하면 딕셔너리의 키가 정렬되어 출력됩니다.
                    기본값은 False로, 이 경우 원래 딕셔너리의 키 순서가 유지됩니다.
    ensure_ascii=False: 이 옵션은 JSON 문자열에서 비-ASCII 문자를 그대로 출력할지를 결정합니다. False로 설정하면, 
                        UTF-8 인코딩으로 비-ASCII 문자가 그대로 포함됩니다. 기본값은 True로, 이 경우 비-ASCII 문자는 Unicode 이스케이프 시퀀스로 변환됩니다.
    '''
        # 파일 쓰기
    #file.write( jsonFile )
        # 파일 닫기
