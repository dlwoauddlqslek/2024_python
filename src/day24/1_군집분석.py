...
'''
3개 리스트: 3개의 군집: 각 군집들의 평균 무게와 당도 표시
[[185.29411765   7.17058824]
 [298.75         8.        ]
 [ 89.23076923   6.55384615]]
'''
# [4] 군집 결과를 확인 #.labels_
print(model.labels_) # 0~7 총8개 # [0 4 4 3 7 3 5 5 5 6 6 2 0 5 0 2 2 2 0 0 4 1 3 4 2 4 1 4 5 5 2 3 6 4 2 6 6 1]
# [5] 결과를 데이터프레임에 추가
df['cluster']=model.labels_
print(df) # 무게와 당도에 따른 군집(번호)을 확인
# [6] 새로운 데이터로 군집 예측
...
plt.scatter(df['weight'],df['sweetness'],c=df['cluster']) # 산점도( x와 y 관계)
plt.scatter(newDf['weight'],newDf['sweetness'],marker='^')
plt.show()

# 개선1: 무게와 당도의 범위가 크다. 스케일 차이가 크다. # 무게는 100단위, 당도 1 단위
...
# 무게와 당도의 스케일 맞추기 # 데이터 표준화 # 무게 100단위, 당도 1단위 # 특정 알고리즘(k평균)에서 무게에 더 큰 비중 갖는다.
    # 여러개 데이터들의 평균을 0으로 하고 표준편차를 1이 되도록 변환 하는 과정: 표준화
...
print(scaledData) # 스케일 후 [-0.81219782 -1.40395189] # 평균에서 떨어진 정도
# [2] 스케일 된 데이터로 모델 학습
model2=KMeans(n_clusters=3)
model2.fit(scaledData) # 스케일 된 데이터를 학습

df['cluster'] = model2.labels_ # 클러스터 결과를 데이터프레임 대입
df['weight_scale']=scaledData[:,0] # 모든 행의 첫번째 열 추출( 스케일 된 무게)
...
# 개선2: 최적의 k(클러스터수)를 찾기 # 엘보방법 # .inertia_
...
for 클러스터수 in range(1,11): # 1~10 까지의 클러스터 수를 테스트 # 10회전
    ...
    print(model.inertia_) # 데이터들의 거리 차이의 제곱값 합을 오차(SSE)를 계산하고 반환한다.
    sse.append(model.inertia_) # SSE를 리스트에 대입
# 총 클러스터 1 부터 10개 까지의 모델 10개 SSE(오차)를 리스트에 저장
print(sse)
# 오차 시각화
