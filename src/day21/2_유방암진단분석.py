...
# 2. 정확도
accuracy=accuracy_score(y_test,y_predict) # 모델이 전체 데이터에서 얼마나 잘 예측 했는지?
# 3. 정밀도
precision=precision_score(y_test,y_predict) # 모델이 양성으로 예측한 것들 중에서 실제 양성 비율
# 4. 재현율
recall=recall_score(y_test,y_predict) # 실제 양성 중에서 모델이 얼마나 잘 양성으로 예측 했는지?
# 5. F1스코어
f1=f1_score(y_test,y_predict) # 정밀도와 재현율의 균형
# 6. ROC 시반 AUC 스코어
roc_auc=roc_auc_score(y_test,y_predict) # 모델이 양성과 음성을 구별하는 능력 평가
# 5가지의 모델 평가 지수 계산 방법 # 1(100%)에 가까울수록 모델은 예측을 잘 표현하고 있다. # 대략 85%
