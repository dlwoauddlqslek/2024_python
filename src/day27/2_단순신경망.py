...
model = tf.keras.Sequential([
    ...
    tf.keras.layers.Dense(1,input_shape=[4]), # 레이어1: 첫번째 레이어가 입력 레이어 # 반듯 input_shape 매개변수를 지정해야 한다.
    tf.keras.layers.Dense(5),  # 레이어2
    tf.keras.layers.Dense(1)   # 레이어3
...
# Trainabble params: 2 # 모델의 업데이트할 파라미터 수 # w 가중치, 편향b 두개이다.
# Non_trainable params: 0 # 모델의 업데이트하지 않을 파라미터 수
    # [2] 컴파일
    # 1. 긴 문자열 지정
...
model.compile(optimizer='sgd',loss='mse',metrics=['mae'])
    # [3] 훈련
history = model.fit(x,y,epochs=1200) # 20번,1200번 반복 훈련
print(history)
    # [4] 시각화
...
# epochs = 20 일 때 x가 10일 때 y= 3 * x + 2 예측값은 # [[32.853497]]
# epochs = 1200 일 때 x가 10일 때 y= 3 * x + 2 예측값은 # [[32.039684]] # 에포크 늘림으로써 더 근사한 값을 예측했다.