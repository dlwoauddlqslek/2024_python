...
# 슬라이싱: 인덱싱을 이용해 요소를 추출, [시작인덱스(포함) : 끝인덱스(포함x) : 증감단위]
print(studentList[0:2])      #['유재석', '강호동'] , 0~2미만
print(studentList[0:3])      #['유재석', '강호동', '신동엽'], 0~3미만
print(studentList[:4])       #['유재석', '강호동', '신동엽', '서장훈'], 생략시 0~4미만
print(studentList[2:])       #['신동엽', '서장훈'], 2~생략시 마지막 인덱스까지
print(studentList[:])        #['유재석', '강호동', '신동엽', '서장훈'], 생략시 0~마지막인덱스
...
# 3. .remove(요소값) : 리스트의 삭제할 요소값이 존재하면 삭제
a.remove(1)
print(a)    #[6,2,3,4]
...
# 5. .count(찾을요소값)     : 리스트의 찾을 요소값이 존재하면 개수 반환
a=[1,2,3,1]
print(a.count(1)) # 2, 리스트내 찾는 요소의 개수 2 반환

# 6. .index(찾을요소값)      : 리스트의 해당 찾을 요소값이 존재하면 인덱스 반환
