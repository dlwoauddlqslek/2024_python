...
# (1) .count('찾을문자') : 문자열내 찾을문자가 존재하면 개수 반환하는 함수
...
print(a.find('가'))      #-1, 존재하지 않으면 -1

# (3) "문자열" .index('찾을문자')  : 문자열내 찾을문자가 존재하면 인덱스 반환 함수
print(a.index('자'))      # 11, 문자열내 '자'인 인덱스 위치
#print(a.index('가'))     # ValueError: substring not found, 존재하지 않으면 예외발생

# (4) "문자" .join("문자열" 또는 리스트)     : 문자열내 문자 사이의 특정문자를 삽입해서 반환 함수
print(','.join(a))      # 코,딩,도, ,헤,매,는, ,만,큼, ,자,기, ,땅,이,야
b=["자바","파이썬","C언어","자바스크립트"]
print('<->'.join(b))    # 자바<->파이썬<->C언어<->자바스크립트, 리스트 요소 마다 특정문자를 삽입 해서 반환 함수
...
# (10) .replace('기존문자','새로운문자') : 문자열내 기존문자가 존재하면 새로운문자로 치환해서 반환 함수
print(a.replace('코딩','파이썬')) # 파이썬도 헤매는 만큼 자기 땅이야

# (11) .split()
# 문자열내 특정문자 기준으로 나누기 해서 리스트로 반환 함수
print(a.split(' '))     # ['코딩도', '헤매는', '만큼', '자기', '땅이야']
print(a.split(' ')[1])  # 헤매는
print(a.split(' ')[3])  # 자기

# 컴퓨터(파이썬,자바,자바스크립트,c언어)에서 문자는 불변 이다.
    # 문자열/정수/실수(리터럴) 수정이 불가능하다.(불변)
e='Python Program'
e.upper()
print(e)
